layui.use(['jquery','layer', 'table','form','upload'], function() {
    ...
    form.verify({  //做表单点击提交时的验证
        ...
        //进行房间号的唯一性验证
        checkRooms:function (value, item) {//value：表单的值、item：表单的DOM对象
            checkRoomNum(value);
            if (!checkRoomNumIf){
                return '客房编号重复';
            }
    ...
    function  loadAllRooms(){
        $.ajax({
            ...
            success:function (data) {
                ...
                var roomStatus1 = "";
                var roomStatus2 = "";
                //通过对data数据集合进行循环，将数据和标签加载到页面(item自定义单条数据对象变量名)
                $.each(data,function (i,item) {
                    if(item.roomStatus=='0'){  //0空闲
                        roomStatus0 += '<li style="background-color: #009688;">';
                        roomStatus0 += '<img class="layui-anim" src="'+qnyName+item.roomPic+'" width="135px" height="135px"/>';
                        ...
                        roomStatus0 += '</div>';
                        roomStatus0 += '</li>';
                    }else if(item.roomStatus=='1'){  //1已入住
                        roomStatus1 += '<li style="background-color: red;">';
                        roomStatus1 += '<img class="layui-anim" src="'+qnyName+item.roomPic+'" width="135px" height="135px"/>';
                        ...
                        roomStatus2 += '<img class="layui-anim" src="'+qnyName+item.roomPic+'" width="135px" height="135px"/>';
    ...
    $("ul").eq(2).on("click","li button",function () {
        ...
        if (event=='del') {   //执行显示状态的修改操作
            ...
            layer.confirm('真将此客房改为空闲状态吗？', function (index) {
    ...
    var uploadInst = upload.render({
        ...
        ,field:"myFile"  ////源文件指定的名字--rooms控制器参数名
        // ,data:{"path":"D:\\web\\img"}  //上传的目标文件夹路径
        ...
        $.ajax({
            ...
            success:function (data) {
                if(data=='success'){
                    ...
                    layer.msg("客房信息删除成功",{icon: 1,time:2000,anim: 4,shade:0.5});
                }else {
                    layer.msg("客房信息删除失败",{icon: 2,time:2000,anim: 5,shade:0.5});
                }
            },
            error:function () {
                layer.msg("服务器异常！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
        ...
        $.ajax({
            ...
            success:function (data) {
                if(data==0){
                    ...
                    checkRoomNumIf = true; //该手机号没有被使用
                }else {
                    layer.tips('此房间号已被使用','#roomNum', {tips: [2,'red'],time:2000});
                    checkRoomNumIf = false; //该手机号已被使用
                }
            },
            error:function () {
                layer.msg("服务器异常！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
